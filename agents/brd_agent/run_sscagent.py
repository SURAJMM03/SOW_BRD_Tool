...
from agents.brd_agent.agents.ssc_agent import create_generation_agent
...
try:
    from orchestration.brd_convo_app.backend.app.section_titles import get_titles as _get_titles
except ImportError:
    try:
        from orchestration.brd_convo_app.backend.app.section_titles import get_titles as _get_titles
    except ImportError:
...
def generate_ssc_sections_from_qa(qa_records: list, research_context: str = "", project_id: str = "") -> dict:
    """
    Generate sections 3, 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7.
    ...
    Auto-extracts images if needed and intelligently injects relevant images per section.
    """
    
    # Image injection is handled upstream in screen3_routes.py — do not duplicate here.
    ...
    if _single_match:
        ...
        gen_prompt = (
            ...
            "You are a senior Blueprint Document writer for enterprise solution "
            "implementations at Bristlecone.\n\n"
            "TASK: Generate sections 3, 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7 for the Blueprint Document.\n\n"
            "RULES:\n"
            ...
            "4. Do NOT hallucinate. Write TBD if a fact is missing.\n"
            ...
            "RESEARCH CONTEXT:\n"
            f"{research_context or 'No context provided.'}\n\n"
            "Generate sections 3, 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7 using <SECTION id=\"X\"> tags only."
        )
