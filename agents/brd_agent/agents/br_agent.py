...
from agents.brd_agent.agents.br_prompt import BENEFIT_REALIZATION_INPUT_COLLECTION_PROMPT, BENEFIT_REALIZATION_GENERATE_PROMPT
...
def _completion(prompt: str, max_tokens: int = 3000, temperature: float = 0.2) -> str:
    """Call Claude for text completion. Matches the signature used by run_bragent."""
    try:
...
class _ClaudeAgent:
    """Minimal LangChain-compatible wrapper so run_bragent.py needs no changes."""
