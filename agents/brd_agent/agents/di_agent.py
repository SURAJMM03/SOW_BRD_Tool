...
from agents.brd_agent.agents.di_prompts import DATA_INTEGRATION_INPUT_COLLECTION_PROMPT, DATA_INTEGRATION_GENERATE_PROMPT
...
def _completion(prompt: str, max_tokens: int = 3000, temperature: float = 0.2) -> str:
    """Call Claude for text completion. Matches the signature used by run_diagent."""
    try:
...
class _ClaudeAgent:
    """Minimal LangChain-compatible wrapper so run_diagent.py needs no changes."""
