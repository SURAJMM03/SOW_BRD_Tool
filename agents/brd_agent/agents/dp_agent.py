...
from agents.brd_agent.agents.dp_prompts import DEMAND_PLANNING_INPUT_COLLECTION_PROMPT, DEMAND_PLANNING_GENERATE_PROMPT
...
def _completion(prompt: str, max_tokens: int = 3000, temperature: float = 0.2) -> str:
    """Call Claude for text completion. Matches the signature used by run_dpagent."""
    try:
...
class _ClaudeAgent:
    """Minimal LangChain-compatible wrapper so run_dpagent.py needs no changes."""
