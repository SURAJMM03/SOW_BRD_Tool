...
from agents.brd_agent.agents.ssc_prompt import SUPPLY_CHAIN_SCOPE_INPUT_COLLECTION_PROMPT, SUPPLY_CHAIN_SCOPE_GENERATE_PROMPT
...
def _completion(prompt: str, max_tokens: int = 3000, temperature: float = 0.2) -> str:
    """Call Claude for text completion. Matches the signature used by run_sscagent."""
    try:
...
class _ClaudeAgent:
    """Minimal LangChain-compatible wrapper so run_sscagent.py needs no changes."""
